#define MODULE_NAME "UAC_TEST_DB"
...
static uac_test_db_t *g_test_db = NULL;
...
int uac_test_db_init(void) {
    ...
    g_shm_fd = shm_open(UAC_TEST_SHM_NAME, O_CREAT | O_RDWR, 0666);
    if (g_shm_fd == -1) {
    ...
    if (ftruncate(g_shm_fd, sizeof(uac_test_db_t)) == -1) {
    ...
    g_test_db = mmap(NULL, sizeof(uac_test_db_t), PROT_READ | PROT_WRITE,
    ...
    if (g_test_db->version != UAC_TEST_DB_VERSION) {
        LOG_INFO("Initializing shared memory database (version %d)", UAC_TEST_DB_VERSION);
        memset(g_test_db, 0, sizeof(uac_test_db_t));
        g_test_db->version = UAC_TEST_DB_VERSION;
...
int uac_test_db_write_result(const uac_test_result_t *result) {
    ...
    // Write the result
    memcpy(&g_test_db->results[slot], result, sizeof(uac_test_result_t));
...
void uac_test_db_close(void) {
    if (g_test_db) {
        munmap(g_test_db, sizeof(uac_test_db_t));
        g_test_db = NULL;
    }
